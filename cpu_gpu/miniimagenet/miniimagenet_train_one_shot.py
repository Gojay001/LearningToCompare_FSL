...
def main():
    ...
    for episode in range(EPISODE):
        ...
        # calculate features
        sample_features = feature_encoder(Variable(samples).to(device)) # 5x64*19*19
        batch_features = feature_encoder(Variable(batches).to(device)) # 75x64*19*19
        ...
        relation_pairs = torch.cat((sample_features_ext,batch_features_ext),2).view(-1,FEATURE_DIM*2,19,19) # 375x128*19*19
